...
# heston_model_3.R
# - MH.imp.part1 ------ Metropolis-Hastings Scheme for partially observed data (start and end)
# - gibbs.ssu.part1 --- Gibbs Sampler for partially observed data (start and end), single-site updates

################################################################################
## New Functions:

MH.imp.part1 <- function(N, initS, initV, Pvec, m){
  nn <- length(initS)
  ...
  store[,1] <- c(initS[1], initV[1])
  store[,nn] <- c(initS[nn], initV[nn])
...
gibbs.ssu.part1 <- function(N, initP, initS, initV, Sigma, a, b, m){
  nn <- length(initS)
  ...
  for (k in 2:N){
    ...
    SVmat <- MH.imp.part1(2, data[1,], data[2,], Pmat[k,], m)
    Smat[k,] <- SVmat$S[2,]
    Vmat[k,] <- SVmat$V[2,]
  }
  return(list(S = Smat,
...
init <- initSVf(Sobs, c(Y[1,2],Y[nrow(Y),2]), m)
initS <- init[,1]
initV <- init[,2]
initP <- rep(0,5)

# Stricter priors for MDB convergence 
a <- rep(-1.2,4)
b <- rep(0.5,4)
...
output.ssu.part1.1 <- gibbs.ssu.part1(2000, initP, initS, initV, Sigma, a, b, m)

postvar <- var(output.ssu.part1.1$P)
lambda <- 0.16
Sigmatune <- lambda*(2.40^2)*postvar/5
system.time(output.ssu.part1.2 <- gibbs.ssu.part1(20000, initP, initS, initV, Sigmatune, a, b, m))
length(unique(output.ssu.part1.2$P[,1]))/20000
# Target AP: 28.39%
# CPU approx: 1200s

par(mfrow=c(1,1))
plot(ts(Y[,1],start=0,frequency=n),ylim=c(min(output.ssu.part1.2$S),max(output.ssu.part1.2$S)), ylab = 'S', main = 'SSU Augmented Time Series, S')
for (i in 0:1600){
  lines(ts(output.ssu.part1.2$S[(4000+10*i),],start=0,frequency=m), col = i)
}
lines(ts(Y[,1],start=0,frequency=n), lwd=2)

plot(ts(Y[,2],start=0,frequency=n),ylim=c(min(output.ssu.part1.2$V),max(output.ssu.part1.2$V)), ylab = 'V', main = 'SSU Augmented Time Series, V')
for (i in 0:1600){
  lines(ts(output.ssu.part1.2$V[(400+10*i),],start=0,frequency=m), col = i)
}
lines(ts(Y[,2],start=0,frequency=n), lwd=2)

out <- output.ssu.part1.2
par(mfrow = c(5,3))
